...
const page = () => {
  ...
  return (
    <div className="w-full px-40 min-h-screen">
      <Navbar/>
      <div className= { isPremium ? ("pt-0 ") : ("pt-40 ") + "w-full flex flex-col items-center justify-center"} >
        { isPremium ? (
          <div className="w-full grid grid-cols-3 pt-8 gap-4">
            ...
            return (
              ...
              <Card className="bordered py-4 border-b-4 hover:border-b cursor-pointer" key={event.id}>
                <div className="aspect-[7/2] p-0 w-full rounded-md border-black overflow-hidden border">
                  <img src={bg.src} alt="Event Background" className="object-cover object-center h-full w-full"/>
                </div>
                <div className="flex flex-col items-start">
                  <div className="badge mb-2">{event.stakeHolder.name}</div>
                  <h1 className="font-bold text-xl mb-4">{event.eventName}</h1>
                  <p className="text-grayy text-sm"><FormatDate>{event.timestamp}</FormatDate></p>
                </div>
              </Card>
            )
          })}
